...
class ProfileRestaurantControllerTest extends AbstractControllerTest {
    ...
    void getAllByDate() throws Exception {
        mockMvc.perform(get(REST_URL + "?date=2023-05-01")
                        .with(TestUtil.userHttpBasic(UserTestData.USER)))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
        assertMatch(restaurantService.getAllWithDish(of(2023, Month.MAY, 1)), Arrays.asList(RestaurantTestData.RESTAURANT1));
    }

    @Test
    void getAllByIdAndByDate() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get(REST_URL + RestaurantTestData.RESTAURANT1_ID + "?date=2023-05-01")
                        .with(TestUtil.userHttpBasic(UserTestData.USER)))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
        assertMatch(restaurantService.getAllForId(of(2023, Month.MAY, 1), RestaurantTestData.RESTAURANT1_ID), Arrays.asList(RestaurantTestData.RESTAURANT1));
    }
